<template>
    <div class="phone_con">
        <div class="phone_title">{{setConfig.title}}</div>
        <div class="phone_screen j_screen" :style="currentPageMain.background "  v-drag-item v-keyboard>
          {{PageList}}
            <div v-for="item in PageList.items" track-by="$index" :id="item.id" :class="item.class" :style="item.style" :title="'ID : '+item.id" :attr="item.attr | json" :type="item.type" :index="$index" @mousedown="selectItemOp($index, $event);" v-change-size v-edit-text-item v-context-menu="'#item_context_menu'">
...
export default {
    ...
    created() { },
    props: ['setConfig', "PageList", "imgList"],
    components: {},
    computed: {},
    mounted() { },
    methods: {
        setBg() {
            this.$emit('showPicBox'); //通知父组件改变。
        },
        saveH5() {
